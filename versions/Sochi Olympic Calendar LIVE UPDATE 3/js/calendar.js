 ...
 **/
	...
	pastPresent = function (test) {
		...
		test.updated ? now = new Date(test.updated) : now = new Date();
		console.log(now + " now");
			...
			calendar.displayData = function (selectedSport, selectedDate, data, arcData) {
				...
				// Retrieving Data From API
					...
					asyncResource(generateURL("summary", params)).done(function (response) {
						...
						// try {
							if (pastPresent({started: sportData.sports[0].event_phases[0].finished_at}) == false && (data.sport != "Ice_Hockey" && data.sport != "Curling")) {
								console.log(1);
								calendar.viewmodel.singleSchedule(true);
							} else if (pastPresent({started: sportData.sports[0].event_phases[0].finished_at}) == true && (data.sport != "Ice_Hockey" && data.sport != "Curling")) {
								console.log(2);
								calendar.viewmodel.singleResult(true);
							} else if (pastPresent({started: sportData.sports[0].event_phases[0].started_at}) == false && (data.sport == "Ice_Hockey")) {
								console.log(3);
								calendar.viewmodel.teamSchedule(true);
							} else if (pastPresent({started: sportData.sports[0].event_phases[0].started_at}) == true && (data.sport == "Ice_Hockey")) {
								console.log(4);
								calendar.viewmodel.teamResult(true);
							} else if (pastPresent({started: sportData.sports[0].event_phases[0].phases[0].started_at, updated: sportData.sports[0].event_phases[0].phases[0].updated_at}) == false && (data.sport == "Curling")) {
								console.log(5);
								calendar.viewmodel.teamSchedule(true);
							} else if (pastPresent({started: sportData.sports[0].event_phases[0].phases[0].started_at, updated: sportData.sports[0].event_phases[0].phases[0].updated_at}) == true && (data.sport == "Curling")) {
								console.log(6);
								calendar.viewmodel.teamResult(true);
							} else {
								console.log(7);
